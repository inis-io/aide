// Package iplocate - API 商城「IP 定位」能力子包。
//
// 能力子包约定：每个能力一个子包，包内暴露 Resource（挂载在 apis.Client 的
// 同名字段上）与能力专属类型（本包为 Result）；共享件（Doer/Error/Receipt/
...
// Result - IP 归属地归一化结果：字段与 proto apis.v1.IPLocateResult 及
...
// ipLocateBody - Query 请求体（{"ip":"…"}；与平台 HTTP 侧 typed 入口同源）
type ipLocateBody struct {
	Ip string `json:"ip"`
}

// Resource - IP 定位能力资源（由 apis 根包 Client 挂载为 lic.Apis.IPLocate）。
type Resource struct {
	// doer - 宿主注入的已认证调用能力
	doer core.Doer
}

// New - 创建 IP 定位能力资源（仅装配宿主调用能力，不发起网络请求）
func New(doer core.Doer) *Resource {
	return &Resource{doer: doer}
}

// Query - IP 归属地查询（typed 便捷方法，高频核心能力）。
// 资源名已含 Locate，方法用 Query 避免 IPLocate.Locate 口吃。
/**
 * @param ctx context.Context - 调用上下文
 * @param ip string - 待查询 IP（IPv4 公网地址）
 * @param requestId ...string - 可选调用级幂等键（缺省自动生成 req_ 前缀键）
 * @return Result - 归属地结果（字段见类型注释）
 * @return core.Receipt - 计量回执
 ...
 * 	}
 */
func (this *Resource) Query(ctx context.Context, ip string, requestId ...string) (Result, core.Receipt, error) {

	body, err := json.Marshal(ipLocateBody{Ip: ip})
	if err != nil {
		return Result{}, core.Receipt{}, err
	}
	raw, err := this.doer.Do(ctx, http.MethodPost, ipLocatePath, body, core.ResolveRequestID(requestId...))
	if err != nil {
		return Result{}, core.Receipt{}, err
	}
	...
	if err = json.Unmarshal(data, &payload); err != nil {
		return Result{}, core.Receipt{}, fmt.Errorf("apis: IP 定位响应解析失败：%w", err)
	}
