...
// 接入最小形态：
//
//	updater, _ := updater.NewUpdater(client, updater.UpdaterOptions{
...
// 订阅器复用 client 的事件拉取与平台公钥（callback.NewEventSubscriber 经 PublicKeys 同源继承）。
func (this *Updater) EventUpdates() *callback.EventSubscriber {
	return callback.NewEventSubscriber(this.client, callback.CallbackOptions{}).
		OnEvent(callback.EventUpdateAvailable, func(ctx context.Context, event *callback.CallbackEvent) (callback.Ack, error) {
			this.checkTick(ctx)
...
//
//	updater, _ := updater.NewUpdater(client, opts)
