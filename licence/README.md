...
IP 定位在 `lic.Apis.IPLocate.Query`，邮件代发在 `lic.Apis.Email.Send`；跨能力语义
（`Invoke` 通用兜底 / `Usage` 只读对账）留在 `apis` 根包。共享核心件（`apis.Error` /
`apis.Receipt` / `apis.ErrNotActivated` / 业务码常量）由 `apis` 根包以类型别名 re-export
（`apis/core` 为唯一实现），能力专属类型（`iplocate.Result` / `email.Input` / `email.Result`）
...
// 2) 邮件代发（email 能力子包）：按实际收件人数计量（每封计 1），正文原样投递
...
| 邮件代发 | `lic.Apis.Email.Send(ctx, email.Input{…}, requestId…)` | `POST /api/v1/apis/mail-send/send` | `ApisRuntimeService.MailSend` |
...
- **邮件代发计量口径**：`Email.Send` 的计量数由服务端算——**声明量** = 剔除空白/空串后的上送条数
...
| `apis/` | API 商城 typed 方法包（按能力子包分类）：根包 `Client{doer, IPLocate, Email}` + `New`（组装子资源）+ 跨能力方法 `Invoke`（通用兜底）/ `Usage`（只读对账）+ 共享件 re-export（`Receipt`/`Error`/业务码常量/`HTTPStatusByCode`/`ErrNotActivated`，别名自 `apis/core`）；`apis/core/` = 共享核心件唯一实现（`Doer`/`Error`+17 业务码/`Receipt`/`ParseEnvelope` 唯一解析点/`ResolveRequestID` 幂等键助手，叶子包只依赖标准库）；`apis/iplocate/` = IP 定位能力（`Resource.Query` + `Result`）；`apis/email/` = 邮件代发能力（`Resource.Send` + `Input`/`Result`）；根包与各能力子包只认 JSON，不 import runtime/proto |
