...
// 配置校验引擎：全系统唯一实现（SDK 本地预校验 + licen-hub backend import 复用，
// 禁止在平台侧复制第二份规则引擎）。覆盖三个校验点：
//  - 定义结构校验（ValidateConfigDefinition / Client.ValidateConfigDefinitions）
//  - 值校验（ValidateConfigValue / Client.ValidateConfig）
...
func (this *Client) ValidateConfig(items map[string]string) []ConfigValidationError {
	...
	for _, key := range slices.Sorted(maps.Keys(items)) {
		...
		rules, err := ParseConfigRuleSet(definition.Rules)
		if err != nil {
			failures = append(failures, ConfigValidationError{Where: key, Message: "定义 rules 解析失败：" + err.Error()})
			continue
		}
		if err = ValidateConfigValue(definition.Type, definition.Options, rules, items[key]); err != nil {
			message := err.Error()
			var validationErr *ConfigValidationError
			if errors.As(err, &validationErr) {
				message = validationErr.Message
			}
			failures = append(failures, ConfigValidationError{Where: key, Message: message})
		}
	}
	return failures
}
...
// 返回 nil 表示全部通过。
func (this *Client) ValidateConfigDefinitions(defs ConfigDefinitions) []ConfigValidationError {
