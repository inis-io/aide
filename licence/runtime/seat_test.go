...
func seedDerivedCaches(t *testing.T, platform *fakePlatform, client *Client) {
	...
	if client.TenantStatus("mall") != LicenceProtocol.StatusValid {
...
func assertSeatReleasedState(t *testing.T, client *Client, dir string) {
	...
	t.Helper()
	if client.Status() != LicenceProtocol.StatusSeatReleased {
...
func forceSeatReleased(platform *fakePlatform) {
	...
	platform.mu.Lock()
	platform.forceStatus = LicenceProtocol.StatusSeatReleased
	platform.mu.Unlock()
}
...
func TestSeatReleasedValidateKeepsCredentials(t *testing.T) {
	...
	for _, item := range statusLog {
		if strings.HasSuffix(item, "->"+LicenceProtocol.StatusSeatReleased) {
			released = true
		}
	}
	if !released {
...
func TestSeatReleasedRestoreStable(t *testing.T) {
	...
	forceSeatReleased(platform)
	time.Sleep(350 * time.Millisecond)
	if first.Status() != LicenceProtocol.StatusSeatReleased {
	...
	defer second.Stop()
	if second.Status() != LicenceProtocol.StatusSeatReleased {
...
func TestSeatReleasedUpdateChannelFailsClosed(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), LicenceProtocol.StatusSeatReleased) {
