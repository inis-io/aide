...
func (this *Client) TenantSync(ctx context.Context, sinceTime int64) (int64, *TenantManifests, error) {
	...
	for _, item := range response.Tenants {
		...
		if LicenceProtocol.PassThrough(item.Status) && len(item.Envelope) > 0 {
...
func (this *Client) TenantValidate(ctx context.Context, tenantCode string, options TenantValidateOptions) (string, error) {
	...
	if LicenceProtocol.PassThrough(response.Status) && len(response.Envelope) > 0 {
...
func (this *Client) TenantStatus(tenantCode string) string {
	...
	return LicenceProtocol.LocalStatus(this.now(), cached.envelope.Payload.ValidUntil, cached.envelope.Payload.GraceDays)
}
...
func (this *Client) TenantFeature(tenantCode string, code string) bool {
	...
	return LicenceProtocol.PassThrough(LicenceProtocol.LocalStatus(this.now(), cached.envelope.Payload.ValidUntil, cached.envelope.Payload.GraceDays)) &&
...
func issueTenant(payload TenantPayload, seed []byte) (TenantEnvelope, error) {
	...
	signature, err := LicenceProtocol.SignPayload(payloadBytes, seed)
	if err != nil {
		return TenantEnvelope{}, err
	}
	return TenantEnvelope{Version: LicenceProtocol.EnvelopeVersion, Algorithm: LicenceProtocol.Algorithm, Payload: payload, Signature: signature}, nil
}
