...
// （LicenceProtocol.PassThrough 口径，与 TenantSync/PullEvents 的 fail-closed 惯例一致）直接返回
...
func TestApisDoerGateNotActivated(t *testing.T) {
	...
	client.mu.Unlock()
	for _, status := range []string{LicenceProtocol.StatusExpired, LicenceProtocol.StatusRevoked, LicenceProtocol.StatusSuspended, LicenceProtocol.StatusSeatReleased, LicenceProtocol.StatusSeatLimitExceeded, LicenceProtocol.StatusInstanceMismatch} {
		client.mu.Lock()
	...
	for _, status := range []string{LicenceProtocol.StatusValid, LicenceProtocol.StatusExpiring, LicenceProtocol.StatusGrace, LicenceProtocol.StatusClockTampered} {
		client.mu.Lock()
		client.state.Status = status
		client.mu.Unlock()
		if _, err = (apisDoer{client: client}).Do(t.Context(), "POST", "/api/v1/anything", nil); errors.Is(err, apis.ErrNotActivated) {
...
func TestApisDoerPassThroughAfterActivation(t *testing.T) {
	...
	if !strings.Contains(string(raw), LicenceProtocol.StatusValid) {
