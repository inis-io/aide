...
func (this *Client) activateLocked(ctx context.Context) error {
	...
	publicKey := hex.EncodeToString(LicenceProtocol.Ed25519PublicKey(seed))
	...
	if code == http.StatusNotFound {
		this.setStatus(LicenceProtocol.StatusNotFound)
	...
	if response.Status != LicenceProtocol.StatusValid && response.Status != LicenceProtocol.StatusExpiring && response.Status != LicenceProtocol.StatusGrace {
...
func (this *Client) validateLocked(ctx context.Context) error {
	...
	if response.Status == LicenceProtocol.StatusExpired {
		this.mu.Lock()
		this.state.ActivationToken = ""
		this.mu.Unlock()
		this.setStatus(LicenceProtocol.StatusExpired)
		this.persist()
		return this.activateLocked(ctx)
	}

	// 席位释放是非自动恢复终态：保留 token/私钥/席位号，清除派生放行与信封缓存。
	if response.Status == LicenceProtocol.StatusSeatReleased {
...
func (this *Client) currentLocked(ctx context.Context) (LicenceProtocol.Envelope, error) {

	uri := "/api/v1/licenses/current?licenseNo=" + this.options.LicenseNo
	code, raw, err := this.doRequest(ctx, http.MethodGet, uri, nil, true)
	if err != nil {
		this.offline()
		return LicenceProtocol.Envelope{}, err
	}
	if code == http.StatusNotFound {
		this.setStatus(LicenceProtocol.StatusNotFound)
		return LicenceProtocol.Envelope{}, errors.New("许可证或实例信息无效")
	}
	...
	this.updateClockOffset(response.ServerTime)
	if response.Status == LicenceProtocol.StatusSeatReleased {
		return LicenceProtocol.Envelope{}, this.seatReleased()
	}

	if response.Status != LicenceProtocol.StatusValid && response.Status != LicenceProtocol.StatusExpiring && response.Status != LicenceProtocol.StatusGrace {
		this.setStatus(response.Status)
		this.persist()
		return LicenceProtocol.Envelope{}, errors.New("当前授权不可用：" + response.Status)
	}
	if err = this.applyEnvelope(response.Envelope); err != nil {
		return LicenceProtocol.Envelope{}, err
	}
...
func (this *Client) seatReleased() error {
	...
	this.mu.Unlock()
	this.setStatus(LicenceProtocol.StatusSeatReleased)
	this.persist()
...
func (this *Client) applyEnvelope(raw json.RawMessage) error {
	...
	envelope, rawPayload, err := LicenceProtocol.ParseEnvelope(raw)
	if err != nil {
		return err
	}
	...
	if envelope.Version != LicenceProtocol.EnvelopeVersion || envelope.Algorithm != LicenceProtocol.Algorithm ||
		!LicenceProtocol.Licence.VerifyRaw(rawPayload, envelope.Signature, publicKey) {
