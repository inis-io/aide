...
func (this *fakePlatform) handlePlatformConfigSync(writer http.ResponseWriter, request *http.Request, body []byte) {
	...
	if params.ProjectId != 0 {
		writeJson(writer, map[string]any{"status": LicenceProtocol.StatusError, "serverTime": time.Now().UnixMilli(), "message": "项目与许可证归属不一致"})
		return
	}
	...
	if err != nil {
		writeJson(writer, map[string]any{"status": LicenceProtocol.StatusError, "message": err.Error()})
		return
	}
	signature, err := LicenceProtocol.SignPayload(rawPayload, this.seed)
	if err != nil {
		writeJson(writer, map[string]any{"status": LicenceProtocol.StatusError, "message": err.Error()})
		return
	}
	writeJson(writer, map[string]any{
		...
		"envelope": PlatformConfigEnvelope{Version: LicenceProtocol.EnvelopeVersion, Algorithm: LicenceProtocol.Algorithm, Payload: payload, Signature: signature},
	})
}
...
func TestPlatformConfigGoldenSignature(t *testing.T) {
	...
	signature, err := LicenceProtocol.SignPayload([]byte(goldenPlatformConfigCanonical), seed)
	if err != nil {
		t.Fatalf("LicenceProtocol.SignPayload 失败: %v", err)
	}
	if signature != goldenPlatformConfigSignature {
		t.Fatalf("签名不一致\n期望: %s\n实际: %s", goldenPlatformConfigSignature, signature)
	}
	if !LicenceProtocol.Licence.VerifySign([]byte(goldenPlatformConfigCanonical), signature, goldenPublicKey) {
		t.Fatalf("验签失败")
	}
}
...
func TestPlatformConfigGoldenTamper(t *testing.T) {
	...
	if LicenceProtocol.Licence.VerifySign(tampered, goldenPlatformConfigSignature, goldenPublicKey) {
		t.Fatalf("篡改后验签竟然通过")
	}
	if LicenceProtocol.Licence.VerifySign([]byte(goldenPlatformConfigCanonical), goldenPlatformConfigSignature, strings.Repeat("00", 32)) {
		t.Fatalf("错误公钥验签竟然通过")
	}
}
