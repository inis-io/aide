...
- 本目录是 Licen Hub 的 Go SDK，按职责拆为「门面根包 + 实现子包」：
  根包 = 纯门面（`doc.go` + `facade.go`，86 个导出符号全部以别名镜像，无任何实现）；
  `runtime/` = 运行面 `Client` + 传输/指纹/安全存储/SaaS/在线更新/发放兑换/配置回推；
  ...
  方法在根包，`IPLocate`/`Email` 按能力子包挂载，见下）。
...
- 依赖纪律：`runtime` 的 gRPC 传输层直接 import `google.golang.org/genproto/googleapis/rpc/errdetails`
  （apis 业务码取 `ErrorInfo.Reason`、闸门明细取 `Metadata`），故该模块位于 `require` 直接依赖块、
  不带 `// indirect` 标记**属正常，勿手动改回**；新增/移除直接 import 后在本目录执行 `go mod tidy` 归一。
...
- 根包（门面）**只允许** import `runtime` 与 `protocol` 两个子包做别名，**禁止**新增任何实现；
- `runtime` → `protocol`（限定引用 `LicenceProtocol.X`，不再导出）、`config`、
  `apis`、`proto/licence/v1`；**禁止** import 根包 / `admin` / `updater` / `callback`；
...
- `apis/core` / `config` 为叶子包，不 import 根包与 `runtime`。apis 面向自声明的
  ...
  **apis 按能力子包分类**：共享核心件（`core.Doer`/`Error` + 17 业务码常量 + `ErrNotActivated`/
  ...
  标准库，打破根包 ↔ 子包依赖环）；每个能力一个子包（`apis/iplocate`/`apis/email`……，包内
  `Resource` + 能力专属类型，共享件一律用 `apis/core`），根 `Client` 把能力 Resource 挂载为
  `lic.Apis` 同名字段（`lic.Apis.IPLocate.Query(ctx, ip)` / `lic.Apis.Email.Send(ctx, input)`），
  ...
  **API 商城四路径已双协议落地（阶段 4；MailSend 为阶段 5 T23）**：typed 方法
  ...
  调用级幂等键由 typed 方法生成/透传，经 context 传到传输层后放 `X-Request-Id` 头 /
  metadata `x-request-id`（`LicenceProtocol.MetadataRequestID`），**不进任何签名 canonical**；
- `proto/licence/v1` 被各包引用，自身不 import 模块内任何包。
- **截至 2026-08-09，运行面与管理面现有全资源均已实现 HTTP + gRPC**（运行面含许可证、在线更新、
  ...
  **API 商城管理面 60 条受控路由（7 个 proto-less `Apis*AdminService`）同步双协议落地，SDK 侧入口为
  ...
  平台登记表）由 `admin/apis_reconcile_test.go` 强制守护，穷尽 60 条路由的 HTTP 假平台与 gRPC bufconn 用例。**
...
- 所有“SDK 客户端 → Licen Hub”平台能力都必须具备 HTTP 与 gRPC 两种实现，包括运行面、
  在线更新控制接口、SaaS 租户运行面、平台配置同步和管理面 `AdminClient` 资源能力。
...
- canonical `.proto` 与生成配置必须只有一个权威来源，Licen Hub 服务端与本 SDK 共同消费；
  具体目录和生成流程以双协议落地方案为准。
- 许可证、租户、平台配置、更新清单等 Ed25519 签名信封必须以原始 JSON 字节在 protobuf `bytes`
...
- **major 红线（≤ v1）**：本 SDK 是 Go module，`v2` 起必须在 import path 追加 `/v2` 后缀、
  全局改引用路径，属破坏性大改。因此 **最大版本号上限为 v1**：永远停留在 `v0.x` / `v1.x`，
  禁止发布 `v2.0.0` 及以上版本。
- **minor 位不随意递增**：`v0.7.0` 中间的 `7`（minor）不得在普通功能迭代中累加。默认建议版本号
  一律为当前版本 **patch 位 `+0.0.1`**（如 `v0.7.0` → `v0.7.1`），把 minor 增速压到最低，
  避免版本号快速逼近 `v1.0.0` 后被迫进入 v2 大改。
- **minor / major 递增仅限用户主动明确要求**：除非用户明说跨版本（如 `+0.1.0`、`+1.0.0`），
  否则每次发 tag 一律在当前版本末位 `+0.0.1`，不自行跨版本——**破坏性更新也不例外**
  （本项目处于活跃开发期，破坏性变更不构成升 minor 的理由）；major 递增上限为 `1`
  （最多到 `v1.0.0`），严禁越级到 v2。
- 发 tag 前先 `git tag --list 'licence/v*'` 核对当前最新版本号，在末位 patch `+1` 后打 tag，
  不凭记忆猜版本号。
